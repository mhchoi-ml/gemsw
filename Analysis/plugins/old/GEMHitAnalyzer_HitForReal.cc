...
private:
  ...
  float b_RecEvAvgCls;
  int b_RecEvChamErr, b_RecEvBigClsEv;

  TTree* t_RecIeta;
  float b_RecIeNrechit;
  float b_RecIeAvgCls;
  int b_RecIeChamErr, b_RecIeBigClsEv;
  int b_RecIeBunchId;
  ...
  int b_RecHitCls;
  int b_RecHitChamErr, b_RecHitBigClsEv;
...
//  hGEMGeom_ = esConsumes<GEMGeometry, MuonGeometryRecord>();
  ...
  RecEvBRANCH(RecEvChamErr, I);
  RecEvBRANCH(RecEvBigClsEv, I);
  ...
  RecIeBRANCH(RecIeChamErr, I);
  RecIeBRANCH(RecIeBigClsEv, I);
  RecIeBRANCH(RecIeBunchId, I);
  ...
  RecHitBRANCH(RecHitChamErr, I);
  RecHitBRANCH(RecHitBigClsEv, I);
...
int GEMHitAnalyzer_HitForReal::maskBigClusterEvent(const edm::Handle<GEMRecHitCollection> gemRecHits) {
...
//  iSetup.get<MuonGeometryRecord>().get(hGEMGeom);
  ...
  */
  ...
  int BigClsEv = maskBigClusterEvent(gemRecHits);
            ...
            for (const GEMChamber* Chamber : SuperChamber->chambers()){
              ...
              for (const GEMEtaPartition* etaPart : Chamber->etaPartitions()){
                ...
                if (RecIeNrechits != 0){
                  ...
                  b_RecIeBigClsEv = BigClsEv;
                  b_RecIeBunchId = iEvent.bunchCrossing();
                  t_RecIeta->Fill();
                }
              } // eta partition loop
            } // chamber loop
      ...
      for (const GEMStation* Station : Region->stations()){
        ...
        for (const GEMRing* Ring : Station->rings()){
          ...
          for (const GEMSuperChamber* SuperChamber : Ring->superChambers()){
            for (const GEMChamber* Chamber : SuperChamber->chambers()){
              GEMDetId chId = Chamber->id();
              b_RecHitChamErr = maskChamberWithError(chId, vfat_status_collection, oh_status_collection);
              int la = chId.layer();
              int ch = chId.chamber();
              for (const GEMEtaPartition* etaPart : Chamber->etaPartitions()){
                GEMDetId ieId = etaPart->id();
