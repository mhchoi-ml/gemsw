#ifndef GEMClsSimAnalyzer_H
#define GEMClsSimAnalyzer_H
...
class GEMClsSimAnalyzer : public edm::one::EDAnalyzer<edm::one::WatchRuns> {  
public:
  explicit GEMClsSimAnalyzer(const edm::ParameterSet&);
  ~GEMClsSimAnalyzer();
...
GEMClsSimAnalyzer::GEMClsSimAnalyzer(const edm::ParameterSet& iConfig)
...
GEMClsSimAnalyzer::~GEMClsSimAnalyzer(){}


// convert a string to enum
GEMClsSimAnalyzer::MatchingMetric GEMClsSimAnalyzer::getMatchingMetric(const std::string name) {
...
// convert a string to enum
GEMClsSimAnalyzer::StartingStateType GEMClsSimAnalyzer::getStartingStateType(const std::string name) {
...
// convert a string to enum
reco::Muon::MuonTrackType GEMClsSimAnalyzer::getMuonTrackType(const std::string name) {
...
GEMClsSimAnalyzer::ScenarioOption GEMClsSimAnalyzer::getScenarioOption(const std::string name) {
...
bool GEMClsSimAnalyzer::isInsideOut(const reco::Track& track) {
  return track.innerPosition().mag2() > track.outerPosition().mag2();
}

void GEMClsSimAnalyzer::buildGEMLayers(const GEMGeometry* gem) {
...
bool GEMClsSimAnalyzer::checkPropagationDirection(const reco::Track* track, const GEMLayer& layer) {
...
GEMClsSimAnalyzer::StartingState GEMClsSimAnalyzer::buildStartingState(
...
GEMClsSimAnalyzer::StartingState GEMClsSimAnalyzer::getInnermostMeasurementState(
...
GEMClsSimAnalyzer::StartingState GEMClsSimAnalyzer::getOutermostMeasurementState(
    const reco::TransientTrack& transient_track) {
  const reco::Track& track = transient_track.track();
...
GEMClsSimAnalyzer::StartingState GEMClsSimAnalyzer::buildStateOnSurfaceWithCSCSegment(
    const reco::Muon& muon, const reco::TransientTrack& transient_track, const GEMLayer& gem_layer) {
  bool found = false;
  TrajectoryStateOnSurface state;
  DetId det_id;
...
GEMClsSimAnalyzer::StartingState GEMClsSimAnalyzer::buildStartingStateAlignmentStyle(
    const reco::Muon& muon, const reco::TransientTrack& transient_track, const GEMLayer& gem_layer) {
  bool found = false;
  TrajectoryStateOnSurface state;
  DetId det_id;
...
// for beam scenario
const CSCSegment* GEMClsSimAnalyzer::findCSCSegmentBeam(const reco::TransientTrack& transient_track,
...
const CSCSegment* GEMClsSimAnalyzer::findCSCSegmentCosmics(const reco::Muon& muon, const GEMLayer& gem_layer) {
  const CSCSegment* best_csc_segment = nullptr;
...
// just thin wrapper
const CSCSegment* GEMClsSimAnalyzer::findCSCSegment(const reco::Muon& muon,
                                                        const reco::TransientTrack& transient_track,
                                                        const GEMLayer& gem_layer) {
  if (kScenario_ == ScenarioOption::kCosmics) {
...
bool GEMClsSimAnalyzer::isMuonSubdetAllowed(const DetId& det_id, const int gem_station) {
...
bool GEMClsSimAnalyzer::isCSCAllowed(const CSCDetId& csc_id, const int gem_station) {
  if ((gem_station < 0) or (gem_station > 2)) {
    edm::LogError("ReadoutError") << "got unexpected gem station " << gem_station;
    return false;
  }
...
bool GEMClsSimAnalyzer::checkBounds(const Plane& plane, const GlobalPoint& global_point) {
...
bool GEMClsSimAnalyzer::checkBounds(const Plane& plane,
...
const GEMEtaPartition* GEMClsSimAnalyzer::findEtaPartition(const GlobalPoint& global_point,
                                                               const GlobalError& global_error,
...
float GEMClsSimAnalyzer::computeRdPhi(const GlobalPoint& prop_global_pos,
...
float GEMClsSimAnalyzer::computeDeltaPhi(const GlobalPoint& prop_global_pos,
                                             const LocalPoint& hit_local_pos,
                                             const GEMEtaPartition* eta_partition) {
...
float GEMClsSimAnalyzer::computeMatchingMetric(const GlobalPoint& prop_global_pos,
                                                   const LocalPoint& hit_local_pos,
                                                   const GEMEtaPartition* eta_partition) {
  float metric;
...
// eta partition.
// std::pair<const GEMRecHit*, float> GEMClsSimAnalyzer::findClosestHit(const GlobalPoint& prop_global_pos,
//                                                                          const GEMRecHitCollection::range& rechit_range,
//                                                                          const GEMEtaPartition* eta_partition) {
std::tuple<const GEMRecHit*, float, int> GEMClsSimAnalyzer::findClosestHit(const GlobalPoint& prop_global_pos,
                                                                         const GEMRecHitCollection::range& rechit_range,
                                                                         const GEMEtaPartition* eta_partition) {
...
bool GEMClsSimAnalyzer::skipGEMStation(const int station) {
  bool skip = false;
...
void
GEMClsSimAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /* GEM Geometry */
...
void GEMClsSimAnalyzer::beginJob(){}
void GEMClsSimAnalyzer::endJob(){}

void GEMClsSimAnalyzer::beginRun(edm::Run const&, edm::EventSetup const& iSetup) {
...
// void GEMClsSimAnalyzer::beginRun(const edm::Run& run, const edm::EventSetup& iSetup) { 
//   /* GEM Geometry */
//   // edm::ESHandle<GEMGeometry> hGEMGeom;
...
void GEMClsSimAnalyzer::endRun(edm::Run const&, edm::EventSetup const&){
}

DEFINE_FWK_MODULE(GEMClsSimAnalyzer);